...
const Experiment = ({ currentStep, changeCurrentStep }) => {
  ...
  return (
    currentOrientation === "portrait-primary" ? <button id="button" onClick={rotate}>Please click to rotate the screen</button> :
      <div className="experiment">
        ...
        <div className="body">
          {currentStep === 'BuildCircuit' ? <> <SlideShow noOfSteps={10} expNo={2} />
            <CodeEditor /></> : <Introduction />}
...
const mapStateToProps = state => ({
  currentStep: state.experimentReducer.currentStep
})
