...
const DigitalExperiment = ({ currentStep, changeCurrentStep }) => {
  ...
  return (
    currentOrientation === "portrait-primary" ? <button id="button" onClick={rotate}>Please click to rotate the screen</button> :
      <div className="experiment">
        <div className="header">
          <Link to="/modules" style={{ display: "flex", alignItems: "center" }}>
            <svg
              width="25"
              height="25"
              viewBox="0 0 25 25"
          ...
          <div onClick={openFullscreen} style={{ cursor: "pointer" }}>
            <svg
              width="26"
              height="25"
              viewBox="0 0 26 25"
        ...
        <div className="body">
          {currentStep === 'BuildCircuit' ? <> <SlideShow noOfSteps={21} expNo={3} />
            <DigitalImages /></> : <Introduction />}
...
const mapStateToProps = state => ({
  currentStep: state.digitalExperimentReducer.currentStep
})
...
export default connect(mapStateToProps, mapDispatchToProps)(DigitalExperiment)
